...
export function ManageAccountsPage() {
  ...
  const { publicKey, nativeBalance, nativeSymbol, tokens, networkInfo } = useWallet();
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Your {networkInfo?.name ?? "wallet"} address and native balance.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2 text-sm">
          ...
          <div className="flex justify-between gap-3">
            ...
            <span className="font-mono">{nativeBalance !== null ? nativeBalance : "—"}</span>
          </div>
        </CardContent>
      </Card>
      ...
      {showTokens && (
        <Card>
          ...
          <CardContent className="space-y-3">
            {tokens.length === 0 ? (
              <p className="text-sm text-muted-foreground">No token accounts with balance.</p>
            ) : (
              tokens.map((token) => (
                <div
                  key={token.mint}
